package org.firstinspires.ftc.teamcode.Stuff_From_Loveland.Detectors;

import android.graphics.Bitmap;

import org.firstinspires.ftc.teamcode.Stuff_From_Loveland.OpenCVPipeline;
...
public class CryptoboxDetector extends OpenCVPipeline {
    ...
    public static Mat processBitmap(Bitmap bitmap, int degrees) {
        Mat tmp = new Mat();
        Utils.bitmapToMat(bitmap, tmp);

        Mat rotMat = Imgproc.getRotationMatrix2D(new Point(tmp.cols()/2, tmp.rows()/2), degrees, 1.0);
        Mat rotated = new Mat();
